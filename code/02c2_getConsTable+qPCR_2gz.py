		...
		if Posnbr in SNPsDict.keys():
			...
			for NTQuery in SNPsDict[Posnbr]['NTQuery']:
				...
				if NTQuery != 'Ins':
				        ...
				        NTList.append(SNPsDict[Posnbr]['NTRef'] + str(Posnbr) + NTQuery + SNPsDict[Posnbr]['NTQuery'][NTQuery][3] + "(" + "{:f}".format(SNPsDict[Posnbr]['NTQuery'][NTQuery][2]) + ")")
		...
		if Posnbr in SNPsDict.keys():
			for NTQuery in SNPsDict[Posnbr]['NTQuery'].keys():
				if NTQuery == "A":
					...
					Insfreq = SNPsDict[Posnbr]['NTQuery'][NTQuery][2]
				else:
					NFreq = SNPsDict[Posnbr]['NTQuery'][NTQuery][2]
			if SNPsDict[Posnbr]['NTRef'] ==  "A":
				Afreq = 1 - (Cfreq + Gfreq + Tfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  "C":
				Cfreq = 1 - (Afreq + Gfreq + Tfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  "G":
				Gfreq = 1 - (Afreq + Cfreq + Tfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  "T":
				Tfreq = 1 - (Afreq + Cfreq + Gfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  ".":
				Insfreq = 1 - (Afreq + Cfreq + Gfreq + Tfreq + NFreq + DelFreq)
			else:
				Nfreq = 1 - (Afreq + Cfreq + Gfreq + Tfreq + InsFreq + DelFreq)
		else:
		...
		if Posnbr in SNPsDict.keys():
			for NTQuery in SNPsDict[Posnbr]['NTQuery'].keys():
				if NTQuery == "A":
					...
					NFreq = SNPsDict[Posnbr]['NTQuery'][NTQuery][2]
			if SNPsDict[Posnbr]['NTRef'] ==  "A":
				Tfreq = 1 - (Cfreq + Gfreq + Tfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  "C":	
				Gfreq = 1 - (Afreq + Gfreq + Tfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  "G":
				Cfreq = 1 - (Afreq + Cfreq + Tfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  "T":
				Afreq = 1 - (Afreq + Cfreq + Gfreq + Delfreq + NFreq + InsFreq)
			elif SNPsDict[Posnbr]['NTRef'] ==  ".":
				Insfreq = 1 - (Afreq + Cfreq + Gfreq + Tfreq + NFreq + DelFreq)
		else:
			if RefGenomeSeq[Posnbr -1 : Posnbr] ==  "A":
...
def getSNPs(GenomeDF, RefGenomeSeq):
	...
	for index, row in GenomeDF.iterrows():
		...
		for muta in MutasList:
				...
				NTRef2 = NT[0] # really found after alignment
				NTQuery = NT[-1]
				AA = muta.split(";")[1]
				Annot = muta.split(";")[2]
				...
				if NTPos not in SNPsDict.keys():
						...
						'Annotation' : Annot 
		                	        ...
		                	        SNPsDict[NTPos] = {'NTRef' : NTRef,
						        'NTQuery' : {'Ins' : [1, AA, 0, NTQuery]}, # number of genomes, aa mutation, frequency of the mutation (initialization using 0) 
						        'Annotation' : Annot 
		                        		}
				else:
				        if NTRef2 != '.':
                                                ...
                                                if 'Ins' not in SNPsDict[NTPos]['NTQuery'].keys():
                                                        SNPsDict[NTPos]['NTQuery']['Ins'] = [1, AA, 0, NTQuery]  # number of genomes, aa mutation, annotation and frequency of the mutation (initialization using 0)
                                                else:
                                                        SNPsDict[NTPos]['NTQuery']['Ins'][0] += 1
		a += 1
